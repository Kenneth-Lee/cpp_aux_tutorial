...
而你用git add命令告诉git你修改了某个文件，这个状态叫“暂存”（Stash）状态。你用
git commit提交一个版本，用的就是这个Stash的状态里面修改的内容。而你仅仅修改了
文件，没有告诉git，这个状态叫unstash状态，表示git不知道你修改了。

你把一个文件修改了，然后add它，这时unstash状态和stash的状态是一致的。但如果add
完了，你又去修改它，这时新的修改是unstash的状态，原来修改过的东西是stash的状态，
你可以再运行git add加到里面去。之后你再做commit，就一次把所有的修改都commit进去
了。git commit -a可以自动把所有unstash的修改都先add了再commit，但它不会增加新写
的文件，只增加修改的文件，所以，如果你有新文件，还是要主动add一次的。

当你没有stash和unstash的内容的之后，你的库就称为Clean的。库和库之间同步，都是用
...
有了unstash的内容了，你按原来的方法，add进去，变成一个commit，这样你本地的
