...
介绍
````
vscode也是一个初学者中比较流行的开发环境，这里也介绍一下，我认为当作一个vim来用
就好了。
...
术。但我个人觉得它稳定性不如vim，所以，这个阶段你让我选，我首选还是学习vim了。
...
时候，可以用文件，也可以用目录作为参数。如果你编辑一个文件，那么vscode就围绕这
一个文件来工作，如果你编辑一个目录，那么vscode就为要着这个目录来工作，这个目录
就称为一个“工作目录”。vscode的很多工作，都是围绕着这个工作目录展开的。这一点就
好像我们后面介绍make的时候一样。我们写程序完成一个任务就是一堆文件，把文件放在
一起就是一个目录了，我们把这个目录叫做一个“工程”（Project），换一个目录就是另一
个工程。所以，这些工具都以一个目录为中心，这是很自然的设计。
...
首先说说命令。code学习了vim的一个长处。因为程序员并不愿意到处去找菜单，而是要下
什么事情，直接下一个命令就可以了。所以vim才会让你按一个冒号就开始输入命令。

code也是一样的，Ctrl-Shift-P可以打开命令窗口，然后你就可以敲命令了。其实Ctrl-P
也是打开命令窗口，只是前者带一个>号，你马上可以输入命令，后者不带这个符号，所以
你输入的，它认为你要打开一个文件。所以，我其实更喜欢只记住Ctrl-P热键，如果我要
输入命令，打开窗口以后加一个>就可以了。

和vim不同的是，vim是要你记住大部分的命令，所以它的命令都很短，而且这些命令都可
以写成脚本组合执行，vscode没指望你记住这些命令，只要你记住几个关键的语句，然后
它来帮你联想，所以它的命令都特别长，比如我们前面举的例子，在工作区创建一个控制
台，就是："Terminal: Create New Terminal in Editor Area"，这个东西你实际试几次
就明白了，用惯这个你是懒得去找菜单的，因为没有这个快。当然，它这个还是不如vim
快，vim退出只要:wq就可以了，它需要输入>Close Window才能完成。

很多程序员写代码的时候不想用鼠标，所以切换窗口都用热键，所以可以记一些这些热键：

* Ctrl-1：跳到第一个编辑器窗口，Ctrl-2是第二个，注意这说的是窗口，不是一个窗口里面的Tab。
  Tab用Ctrl-Tab建来切换。

* Ctrl-Shift-\`：跳到控制台窗口。

* Ctrl-Shift-E：文件浏览器窗口。

其他的自己查吧。

当我们调试的时候，我们巴不得开满窗口，但我们正式写某个函数的时候，只想看到当前
窗口。所以vscode提供了一个禅（Zen）模式，让你好专心写程序，热键是Ctrl-Z K（按完
Ctrl-Z以后再按K），这样全屏只有编辑器一个窗口。

默认按两次esc可以推出禅模式，但如果你安装了vim插件，vim的ESC有特殊用途，这种方
法不好使，如果你不经常做这个动作，就按Alt显示菜单，然后在菜单里面找“查看-外观-
禅模式”退出来。（实际上，如果你安装了vim插件，在编辑器窗口里面，Ctrl-Z K也是不
能用的，你要换一个窗口才能用，但退出你没有窗口可以换。）如果你很喜欢这个功能，
那我建议干脆别用vim插件了，要用vim就专心用vim去算了。
